---
description: 学習分析の機能についてご紹介します．
---

# - 学習分析機能

学習分析は，動画の視聴状況を確認する機能です． 学習者は動画が視聴済みか判断でき，教員は学習者の動画視聴状況の一覧を確認できます．&#x20;

## 学習者の操作方法

学習者は，トピック内の動画を最後まで視聴すると，目次のトピック名の右に， **「完了」** のアイコンが表示されます．

![](../.gitbook/assets/dashboard_01.png)

### 学習分析画面の開き方

1. コースからCHIBI-CHILOを開きます．
2. 画面上部の「学習分析」を選択します．
3. 学習分析の画面が表示されます．

![](../.gitbook/assets/dashboard_02-2.png)

#### 「ブック」タブ

**「ブック」** タブでは学習者の視聴完了状況を確認できます． **「ブック」** の並び順は，Moodleの外部ツールにつけた名前順になります．

* 完了：ブック内のトピックを全て視聴完了している学習者の人数
* 未完了：ブック内のトピックを一部視聴している学習者の人数
* 未視聴：ブック内のトピックを全て視聴していない学習者の人数

{% hint style="info" %}
CHIBI-CHILOに一度もアクセスしていない学習者は，[受講者の同期](dashboard.md#受講者の同期)をすると，未視聴の人数に追加されます．
{% endhint %}

![](../.gitbook/assets/dashboard_03-2.png)

#### 「トピック」タブ

**「トピック」** タブでは，動画の長さや視聴状況の数字が表示されます． また，トピック名をクリックすると動画と平均視聴回数や視聴回数のグラフが表示されます．

![](../.gitbook/assets/dashboard_10.png)

#### 「学習者」タブ

**「学習者」** タブでは，学習者の各トピックの視聴完了状況を確認できます．

* 完了：トピックの動画を全て視聴完了している
* 未完了：トピックの動画を一部視聴している
* 未視聴：トピックの動画を視聴していない
* 繰返視聴：完了状態のトピックの動画を繰り返し視聴している

{% hint style="info" %}
CHIBI-CHILOに一度もアクセスしていない学習者は，[受講者の同期](dashboard.md#受講者の同期)をすると，\[名前未公開]として表示されます．
{% endhint %}

![](../.gitbook/assets/dashboard_04-2.png)

#### 「タグ」タブ

**「タグ」** タブでは，学習者の，各ブックごとのタグの設定状況を確認できます．

![](../.gitbook/assets/dashboard_11.png)

### 受講者の同期

**\[受講者の同期]** ボタンをクリックすることで，コースに受講登録されている学習者の情報が同期して表示されます．

{% hint style="warning" %}
コースの受講登録者を削除した状態で **\[受講者の同期]** ボタンをクリックすると，CHIBI-CHILOの学習者の一覧からも，受講者が削除されますので，ご注意ください．
{% endhint %}

1. **\[受講者の同期]** ボタンをクリックします．
2. **「同期」** をクリックします．

![](../.gitbook/assets/dashboard_07-2.png)

3. 受講者に関する情報が表示されます．

![](../.gitbook/assets/dashboard_08-2.png)

### 視聴分析対象の切り替え

画面上部のドロップダウンメニューで，視聴分析対象の切り替えられます．

* トピックでのすべての活動を分析：ブックに含まれるトピックのうち，他のコースで開いたことのあるトピックも，完了・未完了に含める
* このコースでの活動を分析：ブックに含まれるトピックのうち，他のコースで開いたことがあるが，このコースでは開いたことがないトピックは，完了・未完了に含めない

![](../.gitbook/assets/dashboard_05-2.png)

### 分析データをダウンロード

**\[分析データをダウンロード]** ボタンをクリックします．

{% hint style="info" %}
ダウンロード前に **「分析対象の切り替え」** で選択した視聴分析対象が反映されます．
{% endhint %}

![](../.gitbook/assets/dashboard_06-2.png)

### 全コース視聴分析データの一括ダウンロード

1. ヘッダーの **「ダウンロード」** をクリックします．
2. ダウンロードページへの移動を確認されるので， **「ダウンロードを開始」** をクリックします．
3. **\[全コース視聴分析データをダウンロード]** ボタンをクリックします．
4. csv形式でダウンロードされます．

![](../.gitbook/assets/dashboard_09.png)
